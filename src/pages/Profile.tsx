...
import { profileApi } from '@/services/api'; // Import Service
import {
  User,
  Mail,
  Phone,
...
export const Profile = () => {
  ...
  // Sync form data with user data when available
  useEffect(() => {
    if (user) {
      setFormData({
        phone: user.phone || '',
  ...
  const profileFields = [
    ...
    { icon: Phone, label: 'Phone', value: user?.phone, editable: true, key: 'phone' },
    { icon: MapPin, label: 'Address', value: user?.address, editable: true, key: 'address' },
  ];
        ...
        >
          ...
          {!isEditing ? (
            ...
            <div className="flex gap-2">
              <Button variant="ghost" onClick={() => setIsEditing(false)} disabled={isSaving} className="gap-2">
                <X className="w-4 h-4" />
                Cancel
              </Button>
              <Button onClick={handleSave} variant="gradient" disabled={isSaving} className="gap-2">
          ...
          <div className="gradient-primary p-8 relative">
            ...
            <div className="relative flex items-center gap-6">
              ...
              <div className="text-primary-foreground">
                <h2 className="text-2xl font-bold">{(user?.name === 'User' ? user?.email.split('@')[0] : user?.name) || 'User'}</h2>
                <p className="opacity-90">{user?.position || 'Employee'}</p>
